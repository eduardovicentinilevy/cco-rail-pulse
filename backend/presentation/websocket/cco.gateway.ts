    ...
    socket.on('train:command', async (payload: { trainId?: unknown; command?: unknown; targetBlock?: unknown }) => {
      ...
      try {
        ...
        await operatorRepository.logAudit({
          ...
          action: `EXEC_${rawCommand}`,
          target: `TRAIN_${trainId}`,
          status: 'EXECUTED',
        });

        domainEventBus.emit('train:updated', { lineId, payload: snapshot });
        domainEventBus.emit('system:alert', {
          lineId,
          payload: {
            severity: command === 'HALT' ? 'CRITICAL' : command === 'RESTRICT_SPEED' ? 'WARNING' : 'INFO',
            message: `Comando ${rawCommand} executado no ${trainId} pelo operador ${operator.credential}`,
